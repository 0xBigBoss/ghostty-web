...
export class Terminal implements ITerminalCore {
  ...
  private writeInternal(data: string | Uint8Array, callback?: () => void): void {
    ...
    const needsFullRender = this.needsFullRenderOnWrite(data);
    if (needsFullRender) {
      this.forceFullRender = true;
      this.wasmTerm?.forceRenderRedraw();
    }
    if (typeof normalized === "string" ? normalized.length > 0 : normalized.byteLength > 0) {
  ...
  private needsFullRenderOnWrite(data: string | Uint8Array): boolean {
    if (typeof data === "string") {
      if (data.includes("\b") || data.includes("\r")) return true;
      return this.hasForceFullCsiSequence(data);
    }
    if (data.includes(Terminal.BS_BYTE) || data.includes(0x0d)) {
      return true;
    }
    return this.hasForceFullCsiSequence(data);
  }

  private hasForceFullCsiSequence(data: string | Uint8Array): boolean {
      ...
      if (code === 0x1b && i + 1 < len) {
        ...
        if (next === 0x5b) {
          ...
          while (i < len) {
            ...
            if (c >= 0x40 && c <= 0x7e) {
              if (Terminal.isForceFullCsiFinal(c)) {
                return true;
              }
              break;
            }
  ...
  private static isForceFullCsiFinal(code: number): boolean {
    switch (code) {
