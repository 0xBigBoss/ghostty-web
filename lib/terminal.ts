...
export class Terminal implements ITerminalCore {
  ...
  private static readonly BS_BYTE = 0x08;
  private static readonly CSI_LEFT = "\x1b[D";
  ...
  private writeInternal(data: string | Uint8Array, callback?: () => void): void {
    ...
    const bytes =
      typeof normalized === "string" ? normalized.length : normalized.byteLength;
    if (bytes > 0) {
    ...
    if (needsFullRender && analysis.forceFullReason) {
      profileEvent("bootty:term:force-full-render", {
        reason: analysis.forceFullReason,
        bytes,
        hasBackspace: analysis.hasBackspace,
        hasCarriageReturn: analysis.hasCarriageReturn,
      });
    }
    ...
    if (typeof normalized === "string" && normalized.includes("\x07")) {
      this.bellEmitter.fire();
    } else if (normalized instanceof Uint8Array && normalized.includes(0x07)) {
      this.bellEmitter.fire();
    }
    ...
    if (typeof normalized === "string" && normalized.includes("\x1b]")) {
      this.checkForTitleChange(normalized);
    }
  ...
  private analyzeWriteControls(data: string | Uint8Array): {
    ...
    let forceFullReason: string | null = null;
    while (i < len) {
      const code = typeof data === "string" ? data.charCodeAt(i) : data[i];
      if (code === Terminal.BS_BYTE) hasBackspace = true;
      if (code === 0x0d) hasCarriageReturn = true;
      if (code === 0x1b && i + 1 < len) {
        const next = typeof data === "string" ? data.charCodeAt(i + 1) : data[i + 1];
        if (next === 0x5b) {
          i += 2;
          while (i < len) {
            ...
            if (c >= 0x40 && c <= 0x7e) {
              if (!forceFullReason) {
                forceFullReason = Terminal.getForceFullCsiReason(c);
              }
              break;
            }
            i += 1;
      ...
      i += 1;
      if (forceFullReason && hasBackspace && hasCarriageReturn) {
        break;
      }
    }
    return { forceFullReason, hasBackspace, hasCarriageReturn };
  }

  private static getForceFullCsiReason(code: number): string | null {
    switch (code) {
      case 0x40: // @ (insert char)
        return "csi-insert-char";
      case 0x50: // P (delete char)
        return "csi-delete-char";
      case 0x58: // X (erase char)
        return "csi-erase-char";
      case 0x4a: // J (erase in display)
        return "csi-erase-display";
      case 0x4b: // K (erase in line)
        return "csi-erase-line";
      default:
        return null;
    }
  }
